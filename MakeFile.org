...
LDFLAGS :=
# Mode Debug (mettre a "true" pour activer le mode debug forcé) (identique a la commande: make debug)
DEBUG = false
# Lancer automatiquement gdb après chaque compilation en mode debug ( make exec ! ) ( Valeur true ou false )
...
# Tracage mémoire des variables (no leaks) (mettre a "true" pour activer le mode debug memory forcé) (identique a la commande: make memcheck)
DEBUG_MEMORY = false
...
# On look si on a lancé un gprog
ifneq ($(findstring gprof,$(MAKECMDGOALS)),)
